...
def plot_station_column(
    ...
    iind, eind = (
        np.argmin(np.abs([t - dt.datetime(2023, 10, 14, 14, 10) for t in times])),
        np.argmin(np.abs([t - dt.datetime(2023, 10, 14, 21) for t in times])),
    )
    ...
    if stn_info:
        title = f"({p}) {stn_code} / {stn_info['STATIONNAME']}"
...
def create_dvl_summary(
    date: dt.datetime = dt.datetime(2023, 10, 14),
    stations: List[Tuple[str, Dict[str, str]]] = None,
    outfile: Path = Path("figures/2023/dvl_summary.png"),
):
    ...
    build_legend(fig)
    fig.suptitle(
        f"Doppler Response during 14 Oct 2023 Eclipse",
        fontsize=16,
        fontweight="bold",
        y=0.94,
    )
...
def get_eclipse_info(times, stn_info):
    iind, eind = (
        np.argmin(np.abs([t - dt.datetime(2023, 10, 14, 14, 5) for t in times])),
        np.argmin(np.abs([t - dt.datetime(2023, 10, 14, 20, 55) for t in times]))
    )
    segment_times = times[iind:eind]
    Of = utils.get_eclipse_contours_pyEclipse(
