...
class IonosphereModels:
    ...
    def n_layers(
        ...
        layer_heights: np.ndarray = np.asarray([110., 220.0, 300.]),
        layer_base_ne: np.ndarray = np.asarray([1e11, 4.e11, 8.e11]),
    ...
    def cusp_function_alpha(
        ...
        X, Z, Ne = IonosphereModels.n_layers(
            x, hs, layer_names, layer_heights,
            layer_base_ne, layer_scales, Ne_floor
        )
        alpha_X = np.zeros_like(X)
        alpha_X[X<=x_params[0]] = 1
        alpha_X = np.where(
            (X > x_params[0]) & (X <= x_params[1]),
            1 - d_params[0] * np.sin((np.pi/2)*(X-x_params[0])/(x_params[1]-x_params[0]))**2,
            alpha_X
        )
        alpha_X = np.where(
            ...
            1 - d_params[0] + (
                ...
                np.sin((np.pi/2)*(X-x_params[1])/(x_params[2]-x_params[1]))**2
            ),
            alpha_X
        )
    ...
    def tid_cusp_function_alpha(
        ...
        scale = (
            1 + 
            (
                a*np.exp(-((X + (k*(hm-Z)))/S)**2)
            )
        )
        Ne = calculate_ne_from_plasma_frequency(fp)*\
            (1-((hm-Z)/(2*H_scale))**2)*\
            scale
        Ne[Ne<=0] = 0
        Ne += Ne_floor
