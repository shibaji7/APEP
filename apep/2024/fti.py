...
def generate_fti_profiles(
    ...
    ds.load_data_sets(0, -1, n_jobs=12)
    ...
    for dataset in ds.datasets:
        ...
        f_min, f_max = flim
        frame = frame[
            (frame.frequency >= f_min) & (frame.frequency <= f_max)
            & (frame.range >= 50) & (frame.range <= 400)
        ]
...
if __name__ == "__main__":
    ...
    data_root = Path(
        os.environ.get(
            "VIPIR_SPEED_DEMON_ROOT", "/media/chakras4/Crucial X9/Solar_Eclipse_2024/public/WI937/individual/"
        )
    )
    ...
    src = data_root / "2024" / f"{doy}" / "ionogram"
    ...
    try:
        generate_fti_profiles(
            folder=str(tmp),
            fig_file_name=f"figures/fti.{stn}.{date:%Yj}.png",
            fig_title=None,
            stn=stn,
            flim=(4, 6),
            date=date,
